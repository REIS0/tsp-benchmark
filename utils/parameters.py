...
class Ansatz:
    ...
    def get_ansatz(self, n_qubits: int, ansatz: str, depth: int) -> object:
        ...
        if ansatz == "two_local":
            return TwoLocal(n_qubits, "ry", "cz", "full", reps=depth, name=f"two_local_d_{depth}")
        if ansatz == "pauli_two_design":
            return PauliTwoDesign(n_qubits, reps=depth, name=f"pauli_two_design_d_{depth}")
        if ansatz == "real_amplitudes":
            return RealAmplitudes(n_qubits, reps=depth, name=f"real_amplitudes_d_{depth}")
        ...
        if ansatz == "excitation_preserving":
            return ExcitationPreserving(n_qubits, reps=depth, name=f"excitation_preserving_d_{depth}")
